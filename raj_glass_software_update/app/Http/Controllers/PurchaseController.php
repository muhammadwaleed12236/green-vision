...
class PurchaseController extends Controller
{
    ...
    public function getItems(Request $request)
    {
        ...
        $query = Product::query()->select('id', 'item_name', 'size', 'pcs_in_carton', 'retail_price');
    ...
    public function store_Purchase(Request $request)
{
    ...
    $carton_qtys  = $request->input('carton_qty', []);
    $pcs_arr      = $request->input('pcs', []);
    ...
    $rows = [];
    $totalRows = max(
        count($item_names), count($rates), count($carton_qtys),
        count($pcs_arr), count($amounts), count($gross_totals), count($pcs_carton)
    );

    for ($i = 0; $i < $totalRows; $i++) {
        $name = trim($item_names[$i] ?? '');
        $rate = $rates[$i] ?? null;
        $carton = $carton_qtys[$i] ?? null;
        $pcs = $pcs_arr[$i] ?? null;
    ...
    if (!empty($rowErrors)) {
        ...
        foreach ($rowErrors as $k => $errs) {
            foreach ($errs as $e) $flat[] = "Row ".(intval(substr($k,4))+1).": ".$e;
        }
    ...
    $rates_arr = array_map(function($r){ return $r['rate'] ?? 0; }, $rows);
    $carton_arr = array_map(function($r){ return $r['carton_qty'] ?? 0; }, $rows);
    $pcs_arr2 = array_map(function($r){ return $r['pcs'] ?? 0; }, $rows);
    $gross_arr = array_map(function($r){ return $r['gross_total'] ?? 0; }, $rows);
    $discount_arr = array_map(function($r){ return $r['discount'] ?? 0; }, $rows);
    $amount_arr = array_map(function($r){ return $r['amount'] ?? 0; }, $rows);
    $pcs_carton_arr = array_map(function($r){ return $r['pcs_carton'] ?? 0; }, $rows);
    ...
    $purchaseData = [
        ...
        'carton_qty' => json_encode($carton_arr),
        'pcs' => json_encode($pcs_arr2),
    ...
    foreach ($rows as $r) {
        ...
        $carton_qty = (float)($r['carton_qty'] ?? 0);
        $pcs = (float)($r['pcs'] ?? 0);
        $rate = (float)($r['rate'] ?? 0);
        ...
        $product->carton_quantity = $previous_cartons + $carton_qty;
        $product->initial_stock = $previous_stock + ($carton_qty * $pcs_in_carton) + $pcs;

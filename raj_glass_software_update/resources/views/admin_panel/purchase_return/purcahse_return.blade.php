...
<div class="main-wrapper">
    ...
    <div class="page-wrapper">
        <div class="content">
            <div class="card p-4">
                <h4 class="mb-4 text-center fw-bold text-primary">Purchase Return - Invoice
                    #{{ $purchase->invoice_number }}</h4>
                ...
                <form action="{{ route('purchase.return.store') }}" method="POST">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control wide-input" value="{{ $purchase->purchase_date }}"
                                readonly>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Party Code</label>
                            <input type="text" name="Party_code" class="form-control wide-input"
                                value="{{ $purchase->vendor->Party_code }}" readonly>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Party Name</label>
                            <input type="text" class="form-control wide-input" name="party_name"
                                value="{{ $purchase->party_name }}" readonly>
                        </div>
                        <div class="col-md-3">
                        ...
                        <table class="table table-bordered align-middle text-center">
                            ...
                            <tbody>
                                @foreach($purchase->item as $index => $item)
                                    <tr>
                                        <td style="width: 160px;"><input type="text" name="category[]"
                                                class="form-control wide-input" value="{{ $purchase->category[$index] }}"
                                                readonly></td>
                                        <td style="width: 160px;"><input type="text" name="subcategory[]"
                                                class="form-control wide-input" value="{{ $purchase->subcategory[$index] }}"
                                                readonly></td>
                                        <td style="width: 160px;"><input type="text" name="item[]"
                                                class="form-control wide-input" value="{{ $item }}" readonly></td>
                                        <td style="width: 160px;"><input type="text" name="rate[]"
                                                class="form-control wide-input rate" value="{{ $purchase->rate[$index] }}"
                                                readonly></td>
                                        <td style="width: 160px;"><input type="text" name="carton_qty[]"
                                                class="form-control wide-input" value="{{ $purchase->carton_qty[$index] }}"
                                                readonly></td>

                                        <td style="width: 160px;">
                                            <input type="number" name="return_qty[]"
                                                class="form-control wide-input return-qty" data-index="{{ $index }}"
                                                data-rate="{{ $purchase->rate[$index] ?? 0 }}"
                                                data-pcs="{{ $purchase->pcs_carton[$index] ?? 0 }}"
                                                data-measurement="{{ $purchase->size[$index] ?? 0 }}"
                                                max="{{ $purchase->carton_qty[$index] ?? 0 }}" min="0" value="0" required>

                                        </td>

                                        <td style="width: 160px;">
                                            <input type="text" class="form-control wide-input"
                                                value="{{ $purchase->pcs_carton[$index] }}" readonly>
                                            <input type="hidden" name="pcs_carton[]"
                                                value="{{ $purchase->pcs_carton[$index] }}">
                                        </td>

                                        <!-- measurement (size) -->
                                        <td style="width: 160px;">
                                            <input type="text" class="form-control wide-input"
                                                value="{{ $purchase->size[$index] ?? '' }}" readonly>

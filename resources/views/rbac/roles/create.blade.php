    ...
    <div class="page-wrapper">
        <div class="content">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-white py-3 border-0"><h4 class="fw-bold text-dark mb-0"><i class="fas fa-user-shield text-success me-2"></i>Create Role</h4></div>
                <div class="card-body pt-0">
                    <form method="POST" action="{{ route('rbac.roles.store') }}" id="roleForm">
                        @csrf
                        <div class="row mb-4">
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold text-secondary">Role Name <span class="text-danger">*</span></label>
                                <input type="text" name="name" class="form-control form-control-lg rounded-3" placeholder="e.g. Sales Manager" value="{{ old('name') }}" required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <textarea name="description" class="form-control rounded-3" rows="2" placeholder="Briefly describe this role's purpose...">{{ old('description') }}</textarea>
                            </div>
                        </div>
                                ...
                                <div class="card module-card mb-3 permission-group" data-module="{{ strtolower($module) }}" id="card_{{ Str::slug($module) }}">
                                    <div class="card-header d-flex justify-content-between align-items-center" onclick="toggleModule('{{ Str::slug($module) }}')">
                                        <div class="d-flex align-items-center" onclick="event.stopPropagation()">
                                            @if($parentPerm)
                                                <input type="checkbox" name="permissions[]" value="{{ $parentPerm->id }}" class="select-all me-3" id="selectAll_{{ Str::slug($module) }}" onclick="toggleModulePermissions(this, '{{ Str::slug($module) }}')">
                                            @else
                                    ...
                                    <div class="card-body permissions-body" id="body_{{ Str::slug($module) }}">
                                        <div class="d-flex flex-wrap gap-2">
                                            @foreach($childPerms as $perm)
                                                <label class="permission-badge-item permission-item" id="label_{{ $perm->id }}" data-name="{{ strtolower($perm->name) }}">
                        ...
                        <div class="sticky-bar d-flex justify-content-between align-items-center rounded-4">
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-success px-4 btn-lg rounded-3"><i class="fas fa-save me-1"></i> Create Role</button>

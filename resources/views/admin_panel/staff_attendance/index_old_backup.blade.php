...
<div class="alert alert-warning alert-dismissible fade show" id="pendingAttendanceAlert" role="alert">
    <strong>⚠️ Pending Attendance!</strong>
    <p class="mb-0"><span id="pendingCount">{{ $pendingNotifications->count() }}</span> staff members' attendance is still pending:</p>
    <div id="pendingList" style="margin-top: 10px;">
        @foreach($pendingNotifications as $notif)
            <span class="badge badge-info me-2" data-notification-id="{{ $notif->id }}">
                {{ $notif->staff->name }}
                <button type="button" class="btn-close dismiss-notification" data-notification-id="{{ $notif->id }}" style="font-size: 0.75rem;"></button>
            </span>
        @endforeach
    </div>
</div>
...
<script>
    ...
    function refreshPendingNotifications() {
        $.get("{{ route('attendance.pending-notifications') }}", function(res) {
            if (res.notifications.length === 0) {
                $('#pendingAttendanceAlert').fadeOut(function() { $(this).remove(); });
            } else {
                $('#pendingCount').text(res.notifications.length);
                let html = '';
                res.notifications.forEach(notif => {
                    html += `<span class="badge badge-info me-2" data-notification-id="${notif.id}">
                        ${notif.staff.name}
                        <button type="button" class="btn-close dismiss-notification" data-notification-id="${notif.id}" style="font-size: 0.75rem;"></button>
                    </span>`;
                });
    ...
    setInterval(refreshPendingNotifications, 30000); // Refresh every 30 seconds

    // Dismiss notification for 4 hours
    $(document).on('click', '.dismiss-notification', function(e) {
        e.preventDefault();
        e.stopPropagation();
        let notificationId = $(this).data('notification-id');
        $.post("{{ route('attendance.dismiss-notification') }}", {
    ...
    $('#attendanceForm').on('submit', function (e) {
        ...
        // Check if at least one staff has status
        $('.status').each(function () {
            if ($(this).val() !== '') {
                ...
                if (status === 'present' || status === 'half_day') {
                    let checkIn = tr.find('.ci').val();
                    if (!checkIn) {
                        tr.find('.ci').addClass('is-invalid');
                        hasError = true;
                    }
                }
            }
        ...
        if (hasError) {
            alert('Please fill in required fields for marked staff');
            return;
        }
            ...
            function (res) {
                if (res.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        html: res.message + '<br><small style="color: green;">✓ ' + res.marked_count + ' marked | ⏳ ' + res.pending_count + ' pending</small>',
                        timer: 3000
                    });
